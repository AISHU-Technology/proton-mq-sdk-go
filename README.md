...
## Proton MQ Client
The `ProtonMQClient` interface defines the core method of publish/subscribe messages action.The `NewProtonMQClient` create the client which implemented `ProtonMQClient` according to `mqType`.
...
For example:
```go
client, err := msqclient.NewProtonMQClient("proton-mq-nsq-nsqd.resource", 4151,"proton-mq-nsq-nsqlookupd.resource", 4161, "nsq")
if err != nil {
    log.Fatal("failed to create a proton mq client:", err)
}

// also you can Create ProtonMQClient using configfile
// File content format, please see examples/config.yaml
client, err := msqclient.NewProtonMQClientFromFile("/path/to/configfile")
if err != nil {
    log.Fatal("failed to create a proton mq client:", err)
}
```

Note that it is important to call `Close()` on a `ProtonMQClient` when you do not need to call `Pub(...)`, which intend to close and release the producer connections.
```go
// to publish messages
...
## SASL Support
You can specify options on the `ProtonMQClient` to use SASL authentication, but it will only apply when use kafka.
...
client, err := msqclient.NewProtonMQClient("proton-mq-nsq-nsqd.resource", 4151,"proton-mq-nsq-nsqlookupd.resource", 4161, "nsq", opts...)
