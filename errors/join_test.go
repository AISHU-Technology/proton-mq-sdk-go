...
func TestJoinReturnsNil(t *testing.T) {
	if err := mqerrors.Join(); err != nil {
		t.Errorf("errors.Join() = %v, want nil", err)
	}
	if err := mqerrors.Join(nil); err != nil {
		t.Errorf("errors.Join(nil) = %v, want nil", err)
	}
	if err := mqerrors.Join(nil, nil); err != nil {
...
func TestJoin(t *testing.T) {
	...
	for _, test := range []struct {
		...
		got := mqerrors.Join(test.errs...).(interface{ Unwrap() []error }).Unwrap()
...
func TestJoinErrorMethod(t *testing.T) {
	...
	for _, test := range []struct {
		...
		got := mqerrors.Join(test.errs...).Error()
		if got != test.want {
