...
func TestNewKafkaClient(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			want: &ProtonKafkaClient{brokers: []string{"testPubServer:9092"}},
		},
		{
			name: "Ipv6Broker",
			args: args{
			...
			want: &ProtonKafkaClient{brokers: []string{"[::1]:9092"}},
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := NewKafkaClient(tt.args.pubServer, tt.args.pubPort, tt.args.subServer, tt.args.subPort); !reflect.DeepEqual(got.(*ProtonKafkaClient).brokers, tt.want.(*ProtonKafkaClient).brokers) {
				t.Errorf("NewKafkaClient() = %v, want %v", got.(*ProtonKafkaClient).brokers, tt.want.(*ProtonKafkaClient).brokers)
			}
		})
	}
}

func TestProtonKafkaClientInitialize(t *testing.T) {
	type fields struct {
		username          string
		password          string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err := kc.initialize(); err != nil && err.Error() != tt.wantErr.Error() {
				t.Errorf("ProtonKafkaClient.initialize() error = %v, wantErr %v", err, tt.wantErr)
			}
		})
	}
}

func TestProtonKafkaClientPub(t *testing.T) {
	type fields struct {
		username          string
		password          string
		mechanismProtocol string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err := kc.Pub(tt.args.topic, tt.args.msg); err != nil && !errors.Is(tt.wantErr, err) {
				t.Errorf("ProtonKafkaClient.Pub() error = %v, wantErr %v", err, tt.wantErr)
			}
		})
	}
}

func TestProtonKafkaClientSub(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err := kc.Sub(tt.args.topic, tt.args.channel, tt.args.handler, tt.args.pollIntervalMilliseconds, tt.args.maxInFlight); (err != nil) != tt.wantErr {
				t.Errorf("ProtonKafkaClient.Sub() error = %v, wantErr %v", err, tt.wantErr)
			}
		})
	}
}
